...
export function renderLorenz(container: HTMLElement, counts: number[]): void {
  const size = 240
  const pad = 24
  const svg = el('svg', { viewBox: `0 0 ${size} ${size}`, width: size, height: size }) as SVGSVGElement
  const scale = (v: number) => pad + v * (size - 2 * pad)

  svg.appendChild(el('line', { x1: scale(0), y1: scale(1), x2: scale(1), y2: scale(0), stroke: '#bbb', 'stroke-dasharray': '4 4' }))

  const pts = lorenzPoints(counts).map((p) => `${scale(p.x)},${scale(1 - p.y)}`).join(' ')
  svg.appendChild(el('polyline', { points: pts, fill: 'none', stroke: '#e0245e', 'stroke-width': 2 }))
