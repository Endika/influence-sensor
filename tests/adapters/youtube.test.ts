...
function watched(title: string, videoId: string, channel?: { name: string; id: string }) {
  ...
  if (channel) e.subtitles = [{ name: channel.name, url: `https://www.youtube.com/channel/${channel.id}` }]
  return e
}
...
const searchHistory = [
  { header: 'YouTube', title: 'Has buscado gatos', titleUrl: 'https://www.youtube.com/results?search_query=gatos', time: '2026-05-30T10:00:00Z' },
  { header: 'YouTube', title: 'Has visto un vídeo buscado', titleUrl: 'https://www.youtube.com/watch?v=zzz', time: '2026-05-30T10:01:00Z' },
]
...
describe('youtube adapter', () => {
  ...
  it('turns watch history into channel-attributed interactions', async () => {
    ...
    expect(data.interactions[0].timestamp).toBe(Math.floor(Date.parse('2026-05-31T16:42:57.328Z') / 1000))
  })
